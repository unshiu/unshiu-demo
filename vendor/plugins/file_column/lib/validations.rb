      ...
      def validates_file_format_of(*attrs)
        ...
        validates_each(attrs, options) do |record, attr, value|
          unless value.blank?
            ...
            record.errors.add attr, I18n.t('activerecord.errors.messages.file_invalid_format') unless extensions.include?(value.scan(EXT_REGEXP).flatten.first)
          end
        end
      ...
      def validates_filesize_of(*attrs)  
        ...
        validates_each(attrs, options) do |record, attr, value|
          unless value.blank?
            ...
            record.errors.add attr, I18n.t('activerecord.errors.messages.file_size_greater_than', :count => number_to_human_size(options[:in].first)) if size < options[:in].first
            record.errors.add attr, I18n.t('activerecord.errors.messages.file_size_less_than', :count => number_to_human_size(options[:in].last)) if size > options[:in].last
          end
        end
