...
# 文字コードはexcelでひらかられたときに閲覧できるようにShift-JIS
#
...
def export_csv(start_at, end_at)
  ...
  output_file = FasterCSV.open(EXPORT_PATH + file_name, "w") do |csv|
    csv << [I18n.t('activerecord.attributes.pnt_master.title').tosjis,
            I18n.t('activerecord.attributes.base_user.login').tosjis,
            I18n.t('activerecord.attributes.base_user.email').tosjis,
            I18n.t('activerecord.attributes.pnt_history.difference').tosjis,
            I18n.t('activerecord.attributes.pnt_history.message').tosjis,
            I18n.t('activerecord.attributes.pnt_history.created_at').tosjis,
            I18n.t('activerecord.attributes.pnt_history.updated_at').tosjis]
    unless histories.nil?
      histories.each do |history|
        csv << ["#{history.pnt_point.pnt_master.title.tosjis}",
                "#{history.pnt_point.base_user.login}",
                "#{history.pnt_point.base_user.email}",
                "#{history.difference}",
                "#{history.message.tosjis}",
